...
# author: 임일
# Factorizagion Machines(FM) 구현 (다른 변수 추가)
...
movies = movies[['movie_id', 'title', 'unknown', 'Action', 'Adventure', 'Animation', 
...
# Merge data
x = pd.merge(ratings, movies, how='inner', on='movie_id')
x = pd.merge(x, users, how='inner', on='user_id')
x['age'] = x['age'] / 50                            # Re-scaling
...
K = 250
fm1 = FM(num_x, K, data, y, alpha=0.0002, beta=0.007, train_ratio=0.75, iterations=900, tolerance=0.0005, l2_reg=True, verbose=True)
result = fm1.test()
