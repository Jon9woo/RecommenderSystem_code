...
# 각 rating의 시간을 기록한 full matrix 생성
time_matrix = x_train.pivot(values='timestamp', index='user_id', columns='movie_id')

# 각 사용자의 rating 시간의 차이의 평균을 계산
# for loop 사용
def time_gap_calc1():
    ...
    for i, user in enumerate(tg_matrix):
        for j, other in enumerate(tg_matrix):
            ...
            time_gap[i,j] = np.nanmean(abs((tg_matrix[user] - tg_matrix[other])))
    return time_gap
